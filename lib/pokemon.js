...
export const fetchPokemons = async (offset, limit, sortBy = "id") => {
  try {
    // If sorting by name, we need to fetch more results to get the correct slice
    const fetchLimit = sortBy === "name" ? 1000 : limit
    const response = await fetch(`https://pokeapi.co/api/v2/pokemon?offset=0&limit=${fetchLimit}`)
    const data = await response.json()

    let results = data.results

    // If sorting by name, sort before pagination
    if (sortBy === "name") {
